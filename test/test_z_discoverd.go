...
func (s *ZDiscoverdSuite) TestPromoteDemote(t *c.C) {
	...
	// XXX(jpg): Instead wait for discoverd to come up on the new node
	...
	// We can't do anything Demote API side, maybe just retry with timeout?
	// Or potentionally we could explicitly ask the leader as it will always
	// reflect the true state of the peer list.

	// There should now be only the original peers, additionally make sure our host isn't one of them
	finalPeers, err := s.discoverdClient(t).RaftPeers()
	t.Assert(err, c.IsNil)
