...
func (l *LibvirtLXCBackend) Run(job *host.Job, runConfig *RunConfig) (err error) {
	...
	artifactURI, err := l.maybeResolveDiscoverdURI(job.ImageArtifact.URI)
	if err != nil {
...
// maybeResolveDiscoverdURI resolves a discoverd host in the given URI to an
// address using the configured discoverd URL as the host is likely not using
// discoverd to resolve DNS queries
func (l *LibvirtLXCBackend) maybeResolveDiscoverdURI(uri string) (string, error) {
	u, err := url.Parse(uri)
	if err != nil {
		return "", err
	}
